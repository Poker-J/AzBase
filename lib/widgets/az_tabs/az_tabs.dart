...
/// See also:
///
///  * [AzTabBar], which displays a row of tabs.
///  * [AzTabBarView], which displays a widget for the currently selected tab.
///  * [AzTabBar.indicator], which defines the appearance of the selected tab
///    indicator relative to the tab's bounds.
enum AzTabBarIndicatorSize {
  ...
  ///
  /// This value is used to align the tab's label, typically a [AzTab]
  /// widget's text or icon, with the selected tab indicator.
  label,
}

/// A material design [AzTabBar] tab.
///
...
///  * [AzTabBar], which displays a row of tabs.
///  * [AzTabBarView], which displays a widget for the currently selected tab.
///  * [TabController], which coordinates tab selection between a [AzTabBar] and a [AzTabBarView].
///  * <https://material.io/design/components/tabs.html>
class AzTab extends StatelessWidget {
  /// Creates a material design [AzTabBar] tab.
  ///
...
class _TabStyle extends AnimatedWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextStyle defaultStyle = (labelStyle ?? tabBarTheme.labelStyle ?? themeData.primaryTextTheme.bodyText1)
    ...
    final double _scale = selected ? lerpDouble(multiple, 1, animation.value) : lerpDouble(1, multiple, animation.value);
...
class _IndicatorPainter extends CustomPainter {
  ...
  final AzTabBarIndicatorSize indicatorSize;
  ...
  Rect indicatorRect(Size tabBarSize, int tabIndex) {
    ...
    if (indicatorSize == AzTabBarIndicatorSize.label) {
...
/// See also:
///
///  * [AzTabBarView], which displays page views that correspond to each tab.
class AzTabBar extends StatefulWidget implements PreferredSizeWidget {
  ...
  /// Typically a list of two or more [AzTab] widgets.
  ///
  /// The length of this list must match the [controller]'s [TabController.length]
  /// and the length of the [AzTabBarView.children] list.
  ...
  /// and the entire [AzTabBar] is scrollable. Otherwise each tab gets an equal
  ...
  /// the indicator with the tab's text for [AzTab] widgets and all but the
  /// shortest [AzTab.text] values.
  ///
  ...
  /// is [AzTabBarIndicatorSize.tab] the tab's bounds are as wide as the space
  /// occupied by the tab in the tab bar. If [indicatorSize] is
  /// [AzTabBarIndicatorSize.label], then the tab's bounds are only as wide as
  ...
  /// tab's overall bounds if [indicatorSize] is [AzTabBarIndicatorSize.tab]
  /// (the default) or relative to the bounds of the tab's widget if
  /// [indicatorSize] is [AzTabBarIndicatorSize.label].
  ///
  ...
  /// An optional callback that's called when the [AzTabBar] is tapped.
  ///
  ...
  Size get preferredSize {
    for (final Widget item in tabs) {
      if (item is AzTab) {
        final AzTab tab = item;
        if ((tab.text != null || tab.child != null) && tab.icon != null) return Size.fromHeight(_kTextAndIconTabHeight + indicatorWeight);
      }
    }
  ...
  @override
  _AzTabBarState createState() => _AzTabBarState();
}

class _AzTabBarState extends State<AzTabBar> {
  ...
  @override
  void didUpdateWidget(AzTabBar oldWidget) {
  ...
  Widget build(BuildContext context) {
    ...
    final List<Widget> wrappedTabs = List<Widget>.filled(widget.tabs.length, Center(), growable: true);
...
/// selected tab.
///
/// This widget is typically used in conjunction with a [AzTabBar].
///
/// If a [TabController] is not provided, then there must be a [DefaultTabController]
/// ancestor.
///
/// The tab controller's [TabController.length] must equal the length of the
/// [children] list and the length of the [AzTabBar.tabs] list.
///
/// To see a sample implementation, visit the [TabController] documentation.
class AzTabBarView extends StatefulWidget {
  ...
  /// One widget per tab.
  ///
  /// Its length must match the length of the [AzTabBar.tabs]
...
class _AzTabBarViewState extends State<AzTabBarView> {
  ...
  @override
  void didUpdateWidget(AzTabBarView oldWidget) {
    super.didUpdateWidget(oldWidget);
  ...
  Future<void> _warpToCurrentIndex() async {
    ...
    final int previousIndex = _controller.previousIndex;
    if ((_currentIndex - previousIndex).abs() == 1) return _pageController.animateToPage(_currentIndex, duration: kTabScrollDuration, curve: Curves.ease);
...
class TabPageSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Color fixSelectedColor = selectedColor ?? Theme.of(context).primaryColor;
